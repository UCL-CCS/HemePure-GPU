...
//------------------------------------------------------------------------------
	...
	12	April 2020
		...
		4. Swap the post-collision populations:
					...
					copy hipMemcpyDeviceToDevice.
					The later (hipMemcpyDeviceToDevice) seems to be better. Keep this...
				*** KEEP the hipMemcpyDeviceToDevice option ***
	...
	*/
										...
										site_t lower_limit_Wall, site_t upper_limit_Wall, site_t totalSharedFs, bool write_GlobalMem)
	{
		...
		*/
		...
		momentum_x = (dev_ff[1] - dev_ff[2] + (dev_ff[7]  - dev_ff[8]  + dev_ff[9]  - dev_ff[10] + dev_ff[15] - dev_ff[16] + dev_ff[17] - dev_ff[18]));
		momentum_y = (dev_ff[3] - dev_ff[4] + (dev_ff[7]  + dev_ff[8]  - dev_ff[9]  - dev_ff[10] + dev_ff[11] - dev_ff[12] + dev_ff[13] - dev_ff[14]));
		momentum_z = (dev_ff[5] - dev_ff[6] + (dev_ff[11] + dev_ff[12] - dev_ff[13] - dev_ff[14] + dev_ff[15] + dev_ff[16] - dev_ff[17] - dev_ff[18]));
		*/
		...
		*/
		...
		/*
		// Imperial code: Remember different vector definitions
		double u[3];
		u[0] = velx;
		u[1] = vely;
		u[2] = velz;

		double uSq = u[0]*u[0] + u[1]*u[1] + u[2]*u[2];

		dev_fEq[0]  = nn * ( (1.0f/3.0f ) - (1.0f/2.0f)*uSq );
		dev_fEq[1]  = nn * ( (1.0f/18.0f) + u[0]*( (1.0f/6.0f)  + (1.0f/4.0f)*u[0] ) - (1.0f/12.0f)*uSq );
		dev_fEq[2]  = nn * ( (1.0f/18.0f) - u[0]*( (1.0f/6.0f)  - (1.0f/4.0f)*u[0] ) - (1.0f/12.0f)*uSq );
		dev_fEq[3]  = nn * ( (1.0f/18.0f) + u[1]*( (1.0f/6.0f)  + (1.0f/4.0f)*u[1] ) - (1.0f/12.0f)*uSq );
		dev_fEq[4]  = nn * ( (1.0f/18.0f) - u[1]*( (1.0f/6.0f)  - (1.0f/4.0f)*u[1] ) - (1.0f/12.0f)*uSq );
		dev_fEq[5]  = nn * ( (1.0f/18.0f) + u[2]*( (1.0f/6.0f)  + (1.0f/4.0f)*u[2] ) - (1.0f/12.0f)*uSq );
		dev_fEq[6]  = nn * ( (1.0f/18.0f) - u[2]*( (1.0f/6.0f)  - (1.0f/4.0f)*u[2] ) - (1.0f/12.0f)*uSq );
		dev_fEq[7]  = nn * ( (1.0f/36.0f) + ( u[0]+u[1])*( (1.0f/12.0f) + (1.0f/8.0f)*( u[0]+u[1]) ) - (1.0f/24.0f)*uSq );
		dev_fEq[8]  = nn * ( (1.0f/36.0f) + (-u[0]+u[1])*( (1.0f/12.0f) + (1.0f/8.0f)*(-u[0]+u[1]) ) - (1.0f/24.0f)*uSq );
		dev_fEq[9]  = nn * ( (1.0f/36.0f) + ( u[0]-u[1])*( (1.0f/12.0f) + (1.0f/8.0f)*( u[0]-u[1]) ) - (1.0f/24.0f)*uSq );
		dev_fEq[10] = nn * ( (1.0f/36.0f) + (-u[0]-u[1])*( (1.0f/12.0f) + (1.0f/8.0f)*(-u[0]-u[1]) ) - (1.0f/24.0f)*uSq );
		dev_fEq[11] = nn * ( (1.0f/36.0f) + ( u[1]+u[2])*( (1.0f/12.0f) + (1.0f/8.0f)*( u[1]+u[2]) ) - (1.0f/24.0f)*uSq );
		dev_fEq[12] = nn * ( (1.0f/36.0f) + (-u[1]+u[2])*( (1.0f/12.0f) + (1.0f/8.0f)*(-u[1]+u[2]) ) - (1.0f/24.0f)*uSq );
		dev_fEq[13] = nn * ( (1.0f/36.0f) + ( u[1]-u[2])*( (1.0f/12.0f) + (1.0f/8.0f)*( u[1]-u[2]) ) - (1.0f/24.0f)*uSq );
		dev_fEq[14] = nn * ( (1.0f/36.0f) + (-u[1]-u[2])*( (1.0f/12.0f) + (1.0f/8.0f)*(-u[1]-u[2]) ) - (1.0f/24.0f)*uSq );
		dev_fEq[15] = nn * ( (1.0f/36.0f) + ( u[0]+u[2])*( (1.0f/12.0f) + (1.0f/8.0f)*( u[0]+u[2]) ) - (1.0f/24.0f)*uSq );
		dev_fEq[16] = nn * ( (1.0f/36.0f) + (-u[0]+u[2])*( (1.0f/12.0f) + (1.0f/8.0f)*(-u[0]+u[2]) ) - (1.0f/24.0f)*uSq );
		dev_fEq[17] = nn * ( (1.0f/36.0f) + ( u[0]-u[2])*( (1.0f/12.0f) + (1.0f/8.0f)*( u[0]-u[2]) ) - (1.0f/24.0f)*uSq );
		dev_fEq[18] = nn * ( (1.0f/36.0f) + (-u[0]-u[2])*( (1.0f/12.0f) + (1.0f/8.0f)*(-u[0]-u[2]) ) - (1.0f/24.0f)*uSq );
		*/
		...
		*/
		/*
		//else if( (Ind - upper_limit_Wall +1) * (Ind - upper_limit_MidFluid) <= 0){
		//else if( (Ind >= upper_limit_MidFluid) && ( Ind < upper_limit_Wall)) // mWallCollision
		{
		...
		} // Closes the loop if within the limits for mWallCollision
		*/
