    ...
    namespace iolets
    {
            ...
            state(simulationState), unitConverter(units), bcComms(comms)
      {
        ...
        for (int ioletIndex = 0; ioletIndex < totalIoletCount; ioletIndex++)
        {
          ...
          hemelb::log::Logger::Log<hemelb::log::Debug, hemelb::log::OnePerCore>("BOUNDARYVALUES.CC - Examining Iolet ID: %d : isioletonthisproc? : %d", ioletIndex, isIOletOnThisProc);
          ...
          if (isIOletOnThisProc)
          {
            ...
            hemelb::log::Logger::Log<hemelb::log::Debug, hemelb::log::OnePerCore>("BOUNDARYVALUES.CC - On this Rank -> ioletIndex: %d", ioletIndex);

            // Sharp included the if statement - It never enters though!!!
            // TODO: Check whether the if statement is needed...
            if (iolet->IsCommsRequired()) //DEREK: POTENTIAL MULTISCALE ISSUE (this if-statement)
            {
              ...
              hemelb::log::Logger::Log<hemelb::log::Debug, hemelb::log::OnePerCore>("BOUNDARYVALUES.CC - Rank % d contains the centre of ioletIndex: %d", centreRank, ioletIndex);

              iolet->SetComms(new BoundaryComms(state, centreRank, new_comm));
            } // Sharp included this if statement
          }

          /*
          // From the HIP-CUDA-ROCM branch
...
//            }
      ...
      void BoundaryValues::Reset()
      {
        ...
        // IZ August 2024 
        // Switch to totalIoletCount so that all ranks get the info for all iolets and the corresponding velocityTable..
        // Memory-wise it would be better in the future to do this loop over the local iolets
        for (int i = 0; i < totalIoletCount; i++)
        {
          GetIolets()[i]->Reset(*state);
        }
