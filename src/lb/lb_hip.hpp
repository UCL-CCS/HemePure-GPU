...
*/
				...
				*/
				...
				//cudaStatus = hipMemcpyAsync(&(((distribn_t*)GPUDataAddr_dbl_fOld_b)[nFluid_nodes * LatticeType::NUMVECTORS +1]), &(Data_dbl_fOld_Tr[0]), MemSz, hipMemcpyHostToDevice, stream_memCpy_CPU_GPU_domainEdge);
				// This works as well:
				...
				//cudaStatus = hipMemcpy(&(((distribn_t*)GPUDataAddr_dbl_fOld_b)[nFluid_nodes * LatticeType::NUMVECTORS +1]), &(Data_dbl_fOld_Tr[0]), MemSz, hipMemcpyHostToDevice);
		...
		template<class LatticeType>
			bool LBM<LatticeType>::Read_DistrFunctions_CPU_to_GPU(int64_t firstIndex, int64_t siteCount)
			{
				...
				// Send the data from host (Data_dbl_fOld_Tr) to the Device GPU global memory
				// Memory copy from host (Data_dbl_fOld) to Device (GPUDataAddr_dbl_fOld)
				// cudaStatus = hipMemcpy(GPUDataAddr_dbl_fOld, Data_dbl_fOld, nArray_oldDistr * sizeof(distribn_t), hipMemcpyHostToDevice);
			...
			//
			// If we use hipMemcpy: Remember that from the host perspective the mem copy is synchronous, i.e. blocking
			// so the host will wait the data transfer to complete and then proceed to the next function call

			// Switched to hipMemcpyAsync(): non-blocking on the host,
		...
		template<class LatticeType>
			bool LBM<LatticeType>::Read_DistrFunctions_GPU_to_CPU_totalSharedFs()
			{
				...
				cudaStatus = hipMemcpyAsync(&(fNew_GPU_totalSharedFs[0]), &(((distribn_t*)GPUDataAddr_dbl_fNew_b)[nFluid_nodes * LatticeType::NUMVECTORS]), MemSz, hipMemcpyDeviceToHost, mNet_cuda_stream.Get_stream_memCpy_GPU_CPU_domainEdge_new2() );
				//cudaStatus = hipMemcpyAsync(&(fNew_GPU_totalSharedFs[0]), &(((distribn_t*)GPUDataAddr_dbl_fNew_b)[nFluid_nodes * LatticeType::NUMVECTORS]), MemSz, hipMemcpyDeviceToHost, stream_memCpy_GPU_CPU_domainEdge);

				//cudaStatus = hipMemcpy(&(fNew_GPU_totalSharedFs[0]), &(((distribn_t*)GPUDataAddr_dbl_fNew_b)[nFluid_nodes * LatticeType::NUMVECTORS]), MemSz, hipMemcpyDeviceToHost);
				...
				*/
				...
				// Get the cuda stream created in BaseNet using the class member function Get_stream_memCpy_GPU_CPU_domainEdge_new2():
				hemelb::net::Net& mNet_cuda_stream = *mNet;
				//cudaStatus = hipMemcpyAsync( mLatDat->GetFNew(nFluid_nodes * LatticeType::NUMVECTORS), &(((distribn_t*)GPUDataAddr_dbl_fNew_b)[nFluid_nodes * LatticeType::NUMVECTORS]), MemSz, hipMemcpyDeviceToHost, mNet_cuda_stream.Get_stream_memCpy_GPU_CPU_domainEdge_new2() );
		...
		template<class LatticeType>
			bool LBM<LatticeType>::memCpy_HtD_GPUmem_WallMom(site_t firstIndex, site_t siteCount, std::vector<util::Vector3D<double> >& wallMom_Iolet, void *GPUDataAddr_wallMom)
			{
				...
				printf("\n\n");
				for (int site_i=firstIndex; site_i< (firstIndex+siteCount); site_i++){
					geometry::Site<geometry::LatticeData> site = mLatDat->GetSite(site_i);

					site_t shifted_Fluid_Ind = site_i - firstIndex;

					for (int LB_dir=1; LB_dir<19; LB_dir++){

						double wallMom_x = Data_dbl_WallMom[(LB_dir-1)*siteCount + shifted_Fluid_Ind];
						double wallMom_y = Data_dbl_WallMom[(LB_dir-1)*siteCount + shifted_Fluid_Ind + 1*nArr_wallMom];
						double wallMom_z = Data_dbl_WallMom[(LB_dir-1)*siteCount + shifted_Fluid_Ind + 2*nArr_wallMom];


						// Debugging - Case of Vel BCs - Remove later
						if (site_i==9919 && LB_dir==18){
							if(wallMom_x !=0 || wallMom_y !=0 || wallMom_z !=0)
							printf("Section MemCopy : site_i: %lu, Dir: %d, Wall Mom (x,y,z): (%.5e, %.5e, %.5e) \n", site_i, \
																													LB_dir, wallMom_x, wallMom_y, wallMom_z);
							//printf("Section 4.1 : wallMom_InletWall_Inner / site_i: %lu / Coords(x,y,z): (%ld, %ld, %ld), Dir: %d, Wall Mom (x,y,z): (%.5e, %.5e, %.5e) \n", site_i, site.GetGlobalSiteCoords().x, site.GetGlobalSiteCoords().y, site.GetGlobalSiteCoords().z, \
																															LB_dir, wallMom_x, wallMom_y, wallMom_z);
            }
		...
		template<class LatticeType>
			...
			bool LBM<LatticeType>::Read_Macrovariables_GPU_to_CPU(int64_t firstIndex, int64_t siteCount, lb::MacroscopicPropertyCache& propertyCache)
			{
			  ...
			  //cudaStatus = hipMemcpy(dens_GPU, &(((distribn_t*)GPUDataAddr_dbl_MacroVars)[firstIndex]), MemSz, hipMemcpyDeviceToHost);
			  ...
			  //cudaStatus = hipMemcpyAsync(vx_GPU, &(((distribn_t*)GMem_dbl_MacroVars)[1ULL*nFluid_nodes]), MemSz, hipMemcpyDeviceToHost, stream_Read_Data_GPU_Vel);
			  if(cudaStatus != hipSuccess){
			  ...
			  //cudaStatus = hipMemcpyAsync(vy_GPU, &(((distribn_t*)GMem_dbl_MacroVars)[2ULL*nFluid_nodes]), MemSz, hipMemcpyDeviceToHost, stream_Read_Data_GPU_Vel);
			  if(cudaStatus != hipSuccess){
			  ...
			  //cudaStatus = hipMemcpyAsync(vz_GPU, &(((distribn_t*)GMem_dbl_MacroVars)[3ULL*nFluid_nodes]), MemSz, hipMemcpyDeviceToHost, stream_Read_Data_GPU_Vel);
			  if(cudaStatus != hipSuccess){
		...
		template<class LatticeType>
			bool LBM<LatticeType>::Read_DistrFunctions_GPU_to_CPU_FluidSites()
			{
				...
				//cudaStatus = hipMemcpyAsync(fNew_GPU_b, &(((distribn_t*)GPUDataAddr_dbl_fNew_b)[0]), TotalMem_dbl_fNew_b, hipMemcpyDeviceToHost, stream_Read_distr_Data_GPU);
		...
		template<class LatticeType>
			bool LBM<LatticeType>::Read_DistrFunctions_GPU_to_CPU_tot(int64_t firstIndex, int64_t siteCount, lb::MacroscopicPropertyCache& propertyCache) // Is it necessary to use lb::MacroscopicPropertyCache& propertyCache or just propertyCache, as it is being initialised with the LBM constructor???
			{
				...
				//cudaStatus = hipMemcpyAsync(dens_GPU, &(((distribn_t*)GMem_dbl_MacroVars)[0]), MemSz, hipMemcpyDeviceToHost, stream_Read_Data_GPU_Dens);
		...
		template<class LatticeType>
			bool LBM<LatticeType>::FinaliseGPU()
			{
				...
				//	hipStreamDestroy(stream_Read_distr_Data_GPU);
				...
				hipStreamDestroy(stability_check_stream);
				...
				//hipStreamDestroy(stream_memCpy_GPU_CPU_domainEdge);



				// Free GPU memory
				/*
				cudaStatus = hipFree(GPUDataAddr_dbl_fOld);
				if(cudaStatus != hipSuccess){ fprintf(stderr, "hipFree failed\n"); return false; }

				cudaStatus = hipFree(GPUDataAddr_dbl_fNew);
				if(cudaStatus != hipSuccess){ fprintf(stderr, "hipFree failed\n"); return false; }
				*/
				...
				/*cudaStatus = hipFree(GPUDataAddr_int64_Neigh);
				if(cudaStatus != hipSuccess){ fprintf(stderr, "hipFree failed\n"); return false; }
				*/
...
template<class LatticeType>
					...
					iolets::BoundaryValues* iOutletValues, const util::UnitConverter* iUnits)
			{
				...
				if(cudaStatus != hipSuccess){
					fprintf(stderr, "GPU memory allocation (f_new distr.functions) failed\n");
					initialise_GPU_res = false;
					return initialise_GPU_res;
					//return false;
				}
				...
				if(cudaStatus != hipSuccess){
					fprintf(stderr, "GPU memory transfer (f_old distr.functions) Host To Device failed\n");
					initialise_GPU_res = false;
					return initialise_GPU_res;
					//return false;
				}
				...
				if(cudaStatus != hipSuccess){
					fprintf(stderr, "GPU memory transfer (f_new distr.functions) Host To Device failed\n");
					initialise_GPU_res = false;
					return initialise_GPU_res;
					//return false;
				}
				*/
				...
				//cudaStatus = hipMallocManaged((void**)&(mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat), nArray_Distr * sizeof(distribn_t));
				...
				//cudaStatus = hipMallocManaged((void**)&(mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat), nArray_Distr * sizeof(distribn_t));
      	...
      	site_t site_Count_OutletWall_Inner = mLatDat->GetMidDomainCollisionCount(5);
				...
				*/
				...
				if(hemeIoletBC_Inlet == "LADDIOLET"){
					...
					if(cudaStatus != hipSuccess){
						fprintf(stderr, "GPU memory allocation ghostDensity failed\n");
						initialise_GPU_res = false;
						return initialise_GPU_res;
						//return false;
					}
				...
				*///
				...
				cudaStatus = hipMalloc((void**)&d_Stability_GPU, sizeof(int));
				cudaStatus = hipMalloc((void**)&(mLatDat->d_Stability_GPU_mLatDat), sizeof(int));

				cudaStatus = hipMemcpy(d_Stability_GPU, &h_Stability_GPU, sizeof(int), hipMemcpyHostToDevice);
				if(cudaStatus != hipSuccess){
				...
				cudaStatus = hipMemcpy(mLatDat->d_Stability_GPU_mLatDat, &(mLatDat->h_Stability_GPU_mLatDat), sizeof(int), hipMemcpyHostToDevice);
				if(cudaStatus != hipSuccess){
					fprintf(stderr, "GPU memory transfer Host To Device for Stability param. failed\n");
					initialise_GPU_res = false;
					return initialise_GPU_res;
				...
				hipStreamCreate(&stability_check_stream);
...
/*
				...
				if(nBlocksCalcMacroVars!=0)
					hipLaunchKernelGGL(hemelb::GPU_CalcMacroVars, nBlocksCalcMacroVars, nThreadsCalcMacroVars, 0, stream_ReceivedDistr,  	(distribn_t*)GPUDataAddr_dbl_fOld_b,
...
*/
		...
		template<class LatticeType>
			void LBM<LatticeType>::PreSend()
			{
				...
				//if (myPiD!=0) hipStreamSynchronize(stream_SwapOldAndNew);
				if (myPiD!=0) hipStreamSynchronize(stream_ReceivedDistr);
				...
				if(nBlocks_Check!=0 && mState->GetTimeStep()%1000 ==0){
						hipLaunchKernelGGL(hemelb::GPU_Check_Stability, nBlocks_Check, nThreads_Check, 0, stability_check_stream , 	(distribn_t*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
						...
						hipStreamSynchronize(stability_check_stream);
						// MemCopy from Device To Host the value for the Stability - TODO!!!
						cudaStatus = hipMemcpyAsync( &(mLatDat->h_Stability_GPU_mLatDat), &(((int*)mLatDat->d_Stability_GPU_mLatDat)[0]), sizeof(int), hipMemcpyDeviceToHost, stability_check_stream);
			 ...
			 //--------------------------------------------------------------------------------------------------------------------------
				...
				if(nBlocks_Collide!=0)
					hipLaunchKernelGGL(hemelb::GPU_CollideStream_mMidFluidCollision_mWallCollision_sBB, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreSend_1, 	(distribn_t*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
																									(distribn_t*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
																									(distribn_t*)GPUDataAddr_dbl_MacroVars,
				...
				// nArr_dbl = (mLatDat->GetLocalFluidSiteCount()) = nFluid_nodes
				if(nBlocks_Collide!=0)
					hipLaunchKernelGGL(hemelb::GPU_CollideStream_1_PreReceive_SaveMacroVars, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreSend_1,  (double*)GPUDataAddr_dbl_fOld_b,
				...
				*/
				...
				if(hemeIoletBC_Inlet == "LADDIOLET"){
					...
					site_t site_Count_Inlet_Edge = mLatDat->GetDomainEdgeCollisionCount(2);
					if (site_Count_Inlet_Edge!=0){
						GetWallMom(mInletCollision, start_Index_Inlet_Edge, site_Count_Inlet_Edge, propertyCache); // Fills the propertyCache.wallMom_Cache
						...
						printf("Entering section 1: wallMom_Inlet_Edge \n\n");
					...
					site_t site_Count_InletWall_Edge = mLatDat->GetDomainEdgeCollisionCount(4);
					if (site_Count_InletWall_Edge!=0){
						GetWallMom(mInletWallCollision, start_Index_InletWall_Edge, site_Count_InletWall_Edge, propertyCache); // Fills the propertyCache.wallMom_Cache
						...
						printf("Entering section 2: wallMom_InletWall_Edge \n\n");
					...
					site_t site_Count_Inlet_Inner = mLatDat->GetMidDomainCollisionCount(2);
					if (site_Count_Inlet_Inner!=0){
						GetWallMom(mInletCollision, start_Index_Inlet_Inner, site_Count_Inlet_Inner, propertyCache); // Fills the propertyCache.wallMom_Cache
						...
						//------------------------------------------------------------------
		/*				printf("\nEntering section 3: wallMom_Inlet_Inner with size: %lu \n\n", site_Count_Inlet_Inner);
						for (int site_i=0; site_i<site_Count_Inlet_Inner; site_i++){
							geometry::Site<geometry::LatticeData> site = mLatDat->GetSite(site_i+start_Index_Inlet_Inner);
							for (int LB_dir=0; LB_dir<19; LB_dir++){

								double wallMom_x = wallMom_Inlet_Inner[site_i * LatticeType::NUMVECTORS + LB_dir].x;
								double wallMom_y = wallMom_Inlet_Inner[site_i * LatticeType::NUMVECTORS + LB_dir].y;
								double wallMom_z = wallMom_Inlet_Inner[site_i * LatticeType::NUMVECTORS + LB_dir].z;

								if(wallMom_x !=0 || wallMom_y !=0 || wallMom_z !=0)
									printf("Section 3.1 : wallMom_Inlet_Inner / site_i: %lu, Dir: %d, Wall Mom (x,y,z): (%.5e, %.5e, %.5e) \n", (site_i+start_Index_Inlet_Inner), \
																																LB_dir, wallMom_x, wallMom_y, wallMom_z);
								//printf("Section 3.1 : wallMom_Inlet_Inner / site_i: %lu / Coords(x,y,z): (%ld, %ld, %ld), Dir: %d, Wall Mom (x,y,z): (%.5e, %.5e, %.5e) \n", site_i, site.GetGlobalSiteCoords().x, site.GetGlobalSiteCoords().y, site.GetGlobalSiteCoords().z, \
																																LB_dir, wallMom_x, wallMom_y, wallMom_z);
							}
						}
			*/			//------------------------------------------------------------------
					...
					if (site_Count_InletWall_Inner!=0){
						//propertyCache.wallMom_Cache.SetRefreshFlag(); // Is this needed??? TODO: Check
						GetWallMom(mInletWallCollision, start_Index_InletWall_Inner, site_Count_InletWall_Inner, propertyCache); // Fills the propertyCache.wallMom_Cache
						//propertyCache.wallMom_Cache.SetRefreshFlag(); // Is this needed??? TODO: Check
...
/*						//------------------------------------------------------------------
						printf("\nEntering section 4: wallMom_InletWall_Inner with size: %lu \n\n", site_Count_InletWall_Inner);
						for (int site_i=0; site_i<site_Count_InletWall_Inner; site_i++){
							geometry::Site<geometry::LatticeData> site = mLatDat->GetSite(site_i+start_Index_InletWall_Inner);
							for (int LB_dir=0; LB_dir<19; LB_dir++){

								double wallMom_x = wallMom_InletWall_Inner[site_i * LatticeType::NUMVECTORS + LB_dir].x;
								double wallMom_y = wallMom_InletWall_Inner[site_i * LatticeType::NUMVECTORS + LB_dir].y;
								double wallMom_z = wallMom_InletWall_Inner[site_i * LatticeType::NUMVECTORS + LB_dir].z;

								if(wallMom_x !=0 || wallMom_y !=0 || wallMom_z !=0)
								printf("Section 4.1 : wallMom_InletWall_Inner / site_i: %lu, Dir: %d, Wall Mom (x,y,z): (%.5e, %.5e, %.5e) \n", (site_i+start_Index_InletWall_Inner), \
																															LB_dir, wallMom_x, wallMom_y, wallMom_z);
								//printf("Section 4.1 : wallMom_InletWall_Inner / site_i: %lu / Coords(x,y,z): (%ld, %ld, %ld), Dir: %d, Wall Mom (x,y,z): (%.5e, %.5e, %.5e) \n", site_i, site.GetGlobalSiteCoords().x, site.GetGlobalSiteCoords().y, site.GetGlobalSiteCoords().z, \
																																LB_dir, wallMom_x, wallMom_y, wallMom_z);
							}
	...
	*/					//------------------------------------------------------------------
					...
					//	Pass the ghost density[nInlets] to the GPU kernel (hipMemcpy):
					...
					if (n_Inlets!=0){
						...
						if (myPiD!=0){ // MemCopy hipMemcpyHostToDevice only if rank!=0
							// Memory copy from host (h_ghostDensity) to Device (d_ghostDensity)
							//cudaStatus = hipMemcpy(d_ghostDensity, h_ghostDensity, n_Inlets * sizeof(distribn_t), hipMemcpyHostToDevice);
				...
				if(hemeIoletBC_Outlet == "LADDIOLET"){
					...
					site_t site_Count_Outlet_Edge = mLatDat->GetDomainEdgeCollisionCount(3);
					if (site_Count_Outlet_Edge!=0){
						GetWallMom(mOutletCollision, start_Index_Outlet_Edge, site_Count_Outlet_Edge, propertyCache); // Fills the propertyCache.wallMom_Cache
					...
					site_t site_Count_OutletWall_Edge = mLatDat->GetDomainEdgeCollisionCount(5);
					if (site_Count_OutletWall_Edge!=0){
						GetWallMom(mOutletWallCollision, start_Index_OutletWall_Edge, site_Count_OutletWall_Edge, propertyCache); // Fills the propertyCache.wallMom_Cache
					...
					//	Pass the ghost density_out[nInlets] to the GPU kernel (hipMemcpy):
					...
					if (n_Outlets!=0){ // even rank 0 can "see" this info
						...
						if (myPiD!=0){ // MemCopy hipMemcpyHostToDevice only if rank!=0
							// Memory copy from host (h_ghostDensity) to Device (d_ghostDensity)
							//cudaStatus = hipMemcpy(d_ghostDensity_out, h_ghostDensity_out, n_Outlets * sizeof(distribn_t), hipMemcpyHostToDevice);
				...
				// nArr_dbl =  (mLatDat->GetLocalFluidSiteCount()) is the number of fluid elements that sets how these are organised in memory; see Initialise_GPU (method b - by index LB)
				if(nBlocks_Collide!=0)
					hipLaunchKernelGGL(hemelb::GPU_CollideStream_mWallCollision_sBB_PreRec, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreSend_2, 	(double*)GPUDataAddr_dbl_fOld_b,
																																																															(double*)GPUDataAddr_dbl_fNew_b,
																																																															(double*)GPUDataAddr_dbl_MacroVars,
																																																															(int64_t*)GPUDataAddr_int64_Neigh_d,
																																																															(uint32_t*)GPUDataAddr_uint32_Wall,
				...
				*/
				...
				if (site_Count!=0){
					...
					// Inlets BCs
					if(hemeIoletBC_Inlet == "LADDIOLET"){
						hipLaunchKernelGGL(hemelb::GPU_CollideStream_Iolets_Ladd_VelBCs, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreSend_3,  (distribn_t*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
																														(distribn_t*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
																														(distribn_t*)GPUDataAddr_dbl_MacroVars,
																														(int64_t*)GPUDataAddr_int64_Neigh_d,
						...
						if (n_LocalInlets_mInlet_Edge <=(local_iolets_MaxSIZE/3)){
							hipLaunchKernelGGL(hemelb::GPU_CollideStream_Iolets_NashZerothOrderPressure, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreSend_3, 	(double*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
							...
							hipLaunchKernelGGL(hemelb::GPU_CollideStream_Iolets_NashZerothOrderPressure_v2, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreSend_3, 	(double*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
																															(double*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
																															(double*)GPUDataAddr_dbl_MacroVars,
																															(int64_t*)GPUDataAddr_int64_Neigh_d,
																															(uint32_t*)GPUDataAddr_uint32_Iolet,
				...
				if (site_Count!=0){
					...
					// Inlets BCs
					if(hemeIoletBC_Outlet == "LADDIOLET"){
						hipLaunchKernelGGL(hemelb::GPU_CollideStream_Iolets_Ladd_VelBCs, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreSend_4,  (distribn_t*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
																														(distribn_t*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
																														(distribn_t*)GPUDataAddr_dbl_MacroVars,
																														(int64_t*)GPUDataAddr_int64_Neigh_d,
																														(uint32_t*)GPUDataAddr_uint32_Iolet,
						...
						{
							hipLaunchKernelGGL(hemelb::GPU_CollideStream_Iolets_NashZerothOrderPressure, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreSend_4, 	(double*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
																															(double*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
																															(double*)GPUDataAddr_dbl_MacroVars,
																															(int64_t*)GPUDataAddr_int64_Neigh_d,
																															(uint32_t*)GPUDataAddr_uint32_Iolet,
							...
							hipLaunchKernelGGL(hemelb::GPU_CollideStream_Iolets_NashZerothOrderPressure_v2, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreSend_4, 	(double*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
																															(double*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
																															(double*)GPUDataAddr_dbl_MacroVars,
																															(int64_t*)GPUDataAddr_int64_Neigh_d,
																															(uint32_t*)GPUDataAddr_uint32_Iolet,
				...
				if (site_Count!=0){
					...
					// Inlets BCs
					if(hemeIoletBC_Inlet == "LADDIOLET"){
						hipLaunchKernelGGL(hemelb::GPU_CollideStream_wall_sBB_Iolets_Ladd_VelBCs, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreSend_5, 	(distribn_t*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
	 																												 (distribn_t*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
	 																												 (distribn_t*)GPUDataAddr_dbl_MacroVars,
	 																												 (int64_t*)GPUDataAddr_int64_Neigh_d,
	 																												 (uint32_t*)GPUDataAddr_uint32_Wall,
						...
						{
							hipLaunchKernelGGL(hemelb::GPU_CollideStream_wall_sBB_iolet_Nash, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreSend_5, 	(double*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
																														 (double*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
																														 (double*)GPUDataAddr_dbl_MacroVars,
																														 (int64_t*)GPUDataAddr_int64_Neigh_d,
																														 (uint32_t*)GPUDataAddr_uint32_Wall,
							...
							hipLaunchKernelGGL(hemelb::GPU_CollideStream_wall_sBB_iolet_Nash_v2, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreSend_5, 	(double*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
																														 (double*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
																														 (double*)GPUDataAddr_dbl_MacroVars,
																														 (int64_t*)GPUDataAddr_int64_Neigh_d,
																														 (uint32_t*)GPUDataAddr_uint32_Wall,
				...
				if (site_Count!=0){
					...
					// Inlets BCs
					if(hemeIoletBC_Outlet == "LADDIOLET"){
						hipLaunchKernelGGL(hemelb::GPU_CollideStream_wall_sBB_Iolets_Ladd_VelBCs, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreSend_6, 	(distribn_t*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
	 																												 (distribn_t*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
	 																												 (distribn_t*)GPUDataAddr_dbl_MacroVars,
	 																												 (int64_t*)GPUDataAddr_int64_Neigh_d,
	 																												 (uint32_t*)GPUDataAddr_uint32_Wall,
						...
						{
							hipLaunchKernelGGL(hemelb::GPU_CollideStream_wall_sBB_iolet_Nash, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreSend_6, 	(double*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
																															(double*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
																															(double*)GPUDataAddr_dbl_MacroVars,
																															(int64_t*)GPUDataAddr_int64_Neigh_d,
																															(uint32_t*)GPUDataAddr_uint32_Wall,
							...
							hipLaunchKernelGGL(hemelb::GPU_CollideStream_wall_sBB_iolet_Nash_v2, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreSend_6, 	(double*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
																															(double*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
																															(double*)GPUDataAddr_dbl_MacroVars,
																															(int64_t*)GPUDataAddr_int64_Neigh_d,
																															(uint32_t*)GPUDataAddr_uint32_Wall,
				...
				*/
					...
					// Delete the variables used for hipMemcpy
		...
		template<class LatticeType>
			void LBM<LatticeType>::PreReceive()
			{
				...
				*/
				...
				// nArr_dbl =  (mLatDat->GetLocalFluidSiteCount()) is the number of fluid elements that sets how these are organised in memory; see Initialise_GPU (method b - by index LB)
				if(nBlocks_Collide!=0)
					hipLaunchKernelGGL(hemelb::GPU_CollideStream_mMidFluidCollision_mWallCollision_sBB, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreRec_1, 	(distribn_t*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
																									(distribn_t*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
																									(distribn_t*)GPUDataAddr_dbl_MacroVars,
																									(site_t*)GPUDataAddr_int64_Neigh_d,
																									(uint32_t*)GPUDataAddr_uint32_Wall,
				...
				// nArr_dbl =  (mLatDat->GetLocalFluidSiteCount()) is the number of fluid elements that sets how these are organised in memory; see Initialise_GPU (method b - by index LB)
				if(nBlocks_Collide!=0)
					hipLaunchKernelGGL(hemelb::GPU_CollideStream_1_PreReceive_SaveMacroVars, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreRec_1, 	(double*)GPUDataAddr_dbl_fOld_b,
																									(double*)GPUDataAddr_dbl_fNew_b,
																									(double*)GPUDataAddr_dbl_MacroVars,
																									(int64_t*)GPUDataAddr_int64_Neigh_d,
																									(mLatDat->GetLocalFluidSiteCount()),
				...
				*/
				...
				if(nBlocks_Collide!=0)
					hipLaunchKernelGGL(hemelb::GPU_CollideStream_mWallCollision_sBB_PreRec, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreRec_2, 	(double*)GPUDataAddr_dbl_fOld_b,
																						(double*)GPUDataAddr_dbl_fNew_b,
																						(double*)GPUDataAddr_dbl_MacroVars,
																						(int64_t*)GPUDataAddr_int64_Neigh_d,
																						(uint32_t*)GPUDataAddr_uint32_Wall,
				...
				*/
				...
				if (site_Count!=0){
					...
					// Inlets BCs
					if(hemeIoletBC_Inlet == "LADDIOLET"){
						hipLaunchKernelGGL(hemelb::GPU_CollideStream_Iolets_Ladd_VelBCs, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreRec_3,  (distribn_t*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
																														(distribn_t*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
																														(distribn_t*)GPUDataAddr_dbl_MacroVars,
																														(int64_t*)GPUDataAddr_int64_Neigh_d,
																														(uint32_t*)GPUDataAddr_uint32_Iolet,
						...
						{
							hipLaunchKernelGGL(hemelb::GPU_CollideStream_Iolets_NashZerothOrderPressure, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreRec_3, 	(double*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
																															(double*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
																															(double*)GPUDataAddr_dbl_MacroVars,
																															(int64_t*)GPUDataAddr_int64_Neigh_d,
																															(uint32_t*)GPUDataAddr_uint32_Iolet,
							...
							hipLaunchKernelGGL(hemelb::GPU_CollideStream_Iolets_NashZerothOrderPressure_v2, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreRec_3, 	(double*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
																										(double*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
																										(double*)GPUDataAddr_dbl_MacroVars,
																										(int64_t*)GPUDataAddr_int64_Neigh_d,
																										(uint32_t*)GPUDataAddr_uint32_Iolet,
				...
				if (site_Count!=0){
					...
					// Inlets BCs
					if(hemeIoletBC_Outlet == "LADDIOLET"){
						hipLaunchKernelGGL(hemelb::GPU_CollideStream_Iolets_Ladd_VelBCs, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreRec_4,  (distribn_t*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
																														(distribn_t*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
																														(distribn_t*)GPUDataAddr_dbl_MacroVars,
																														(int64_t*)GPUDataAddr_int64_Neigh_d,
																														(uint32_t*)GPUDataAddr_uint32_Iolet,
						...
						{
							hipLaunchKernelGGL(hemelb::GPU_CollideStream_Iolets_NashZerothOrderPressure, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreRec_4, 	(double*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
																															(double*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
																															(double*)GPUDataAddr_dbl_MacroVars,
																															(int64_t*)GPUDataAddr_int64_Neigh_d,
																															(uint32_t*)GPUDataAddr_uint32_Iolet,
							...
							hipLaunchKernelGGL(hemelb::GPU_CollideStream_Iolets_NashZerothOrderPressure_v2, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreRec_4, 	(double*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
																															(double*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
																															(double*)GPUDataAddr_dbl_MacroVars,
																															(int64_t*)GPUDataAddr_int64_Neigh_d,
																															(uint32_t*)GPUDataAddr_uint32_Iolet,
				...
				if (site_Count!=0){
					...
					// Inlets BCs
					if(hemeIoletBC_Inlet == "LADDIOLET"){
						hipLaunchKernelGGL(hemelb::GPU_CollideStream_wall_sBB_Iolets_Ladd_VelBCs, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreRec_5, 	(distribn_t*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
	 																												 (distribn_t*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
	 																												 (distribn_t*)GPUDataAddr_dbl_MacroVars,
	 																												 (int64_t*)GPUDataAddr_int64_Neigh_d,
	 																												 (uint32_t*)GPUDataAddr_uint32_Wall,
						...
						{
							hipLaunchKernelGGL(hemelb::GPU_CollideStream_wall_sBB_iolet_Nash, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreRec_5, 	(double*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
																															(double*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
																															(double*)GPUDataAddr_dbl_MacroVars,
																															(int64_t*)GPUDataAddr_int64_Neigh_d,
																															(uint32_t*)GPUDataAddr_uint32_Wall,
							...
							hipLaunchKernelGGL(hemelb::GPU_CollideStream_wall_sBB_iolet_Nash_v2, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreRec_5, 	(double*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
																															(double*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
																															(double*)GPUDataAddr_dbl_MacroVars,
																															(int64_t*)GPUDataAddr_int64_Neigh_d,
																															(uint32_t*)GPUDataAddr_uint32_Wall,
				...
				if (site_Count!=0){
					...
					// Inlets BCs
					if(hemeIoletBC_Outlet == "LADDIOLET"){
						hipLaunchKernelGGL(hemelb::GPU_CollideStream_wall_sBB_Iolets_Ladd_VelBCs, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreRec_6, 	(distribn_t*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
	 																												 (distribn_t*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
	 																												 (distribn_t*)GPUDataAddr_dbl_MacroVars,
	 																												 (int64_t*)GPUDataAddr_int64_Neigh_d,
	 																												 (uint32_t*)GPUDataAddr_uint32_Wall,
						...
						{
							hipLaunchKernelGGL(hemelb::GPU_CollideStream_wall_sBB_iolet_Nash, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreRec_6, 	(double*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
																															(double*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
																															(double*)GPUDataAddr_dbl_MacroVars,
																															(int64_t*)GPUDataAddr_int64_Neigh_d,
																															(uint32_t*)GPUDataAddr_uint32_Wall,
							...
							hipLaunchKernelGGL(hemelb::GPU_CollideStream_wall_sBB_iolet_Nash_v2, nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreRec_6, 	(double*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
																															(double*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
																															(double*)GPUDataAddr_dbl_MacroVars,
																															(int64_t*)GPUDataAddr_int64_Neigh_d,
																															(uint32_t*)GPUDataAddr_uint32_Wall,
				...
				//if(myPiD!=0) hipStreamSynchronize(stream_memCpy_GPU_CPU_domainEdge);
				...
				if(myPiD!=0 && mState->GetTimeStep()%1000 ==0){
						...
						// MemCopy from Device To Host the value for the Stability - TODO!!!
						// cudaStatus = hipMemcpyAsync( &(mLatDat->h_Stability_GPU_mLatDat), &(((int*)mLatDat->d_Stability_GPU_mLatDat)[0]), sizeof(int), hipMemcpyDeviceToHost, stability_check_stream);
						cudaStatus = hipMemcpy( &(mLatDat->h_Stability_GPU_mLatDat), &(((int*)mLatDat->d_Stability_GPU_mLatDat)[0]), sizeof(int), hipMemcpyDeviceToHost);
...
*/
		...
		template<class LatticeType>
			void LBM<LatticeType>::PostReceive()
			{
				...
				/** 8-7-2020:
						Maybe remove the synch point: hipStreamSynchronize(stream_memCpy_CPU_GPU_domainEdge);
				...
				*/
				...
				if(myPiD!=0) {
					// hipStreamSynchronize(stream_memCpy_CPU_GPU_domainEdge); // Needed if we switch to asynch memcopy and use this stream in Read_DistrFunctions_CPU_to_GPU_totalSharedFs
				...
				*/
				...
				if (nBlocks_StreamRecDistr!=0)
					hipLaunchKernelGGL(hemelb::GPU_StreamReceivedDistr, nBlocks_StreamRecDistr, nThreads_StreamRecDistr, 0, stream_ReceivedDistr,  (distribn_t*)mLatDat->GPUDataAddr_dbl_fOld_b_mLatDat,
																																																	(distribn_t*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
		...
		template<class LatticeType>
			void LBM<LatticeType>::EndIteration()
			{
			  ...
			  int myPiD = rank_Com.Rank();
				...
				if(nBlocks_Swap!=0)
					hipLaunchKernelGGL(hemelb::GPU_SwapOldAndNew, nBlocks_Swap, nThreads_Swap, 0, stream_ReceivedDistr,  (double*)GPUDataAddr_dbl_fOld_b, (double*)GPUDataAddr_dbl_fNew_b, site_Count, offset, (offset + site_Count));
					//hipLaunchKernelGGL(hemelb::GPU_SwapOldAndNew, nBlocks_Swap, nThreads_Swap, 0, stream_SwapOldAndNew,  (double*)GPUDataAddr_dbl_fOld_b, (double*)GPUDataAddr_dbl_fNew_b, site_Count, offset, (offset + site_Count));
				// End of Approach 1
				*/
				...
				//========================================================================================================
				// Approach 2: Using hipMemcpyDeviceToDevice:
				// As this is a single large copy from device global memory to device global memory, then  hipMemcpyDeviceToDevice should be ok.
				...
				*/
				...
				if (myPiD!=0)
				{
					hipDeviceSynchronize(); // Included a hipStreamSynchronize at the beginning of PreSend(); Should do the same job
